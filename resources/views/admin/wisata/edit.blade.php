        ...
        <div class="row justify-content-center">
            <div class="col-lg-10">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h4 class="fw-bold mb-0">Edit Destinasi Wisata</h4>
                    ...
                    <div class="card-body p-4">
                        <form action="{{ route('admin.wisata.update', $destination->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <small class="text-muted">Ketik untuk mencari lokasi baru jika ingin mengubah.</small>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="d-flex gap-2 mb-2">
                                    ...
                                    <button type="button" class="btn btn-sm btn-outline-secondary" id="resetMap"><i class="fas fa-sync-alt"></i> Reset ke Posisi Awal</button>
                                </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" 
                                          id="description" name="description" rows="4" required>{{ old('description', $destination->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('facilities') is-invalid @enderror" 
                                          id="facilities" name="facilities" rows="3">{{ old('facilities', $destination->facilities) }}</textarea>
                                @error('facilities')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="row">
                                {{-- Harga Tiket --}}
                                <div class="col-md-6 mb-3">
                                    <label for="price_per_person" class="form-label fw-semibold">Harga per Orang (Rp) <span class="text-danger">*</span></label>
                                    <input type="number" class="form-control @error('price_per_person') is-invalid @enderror" 
                                           ...
                                           value="{{ old('price_per_person', $destination->price_per_person) }}" required>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" class="form-control @error('parking_price') is-invalid @enderror" 
                                           ...
                                           value="{{ old('parking_price', $destination->parking_price) }}">
                                    @error('parking_price')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="mb-3">
                                ...
                                @php
                                    ...
                                    // Jika berupa array (karena json_decode di model), ubah jadi string
                                    if (is_array($activities)) {
                                        $activities = implode(', ', $activities);
                                    }
                                @endphp
                                <textarea class="form-control @error('popular_activities') is-invalid @enderror" 
                                          id="popular_activities" name="popular_activities" rows="2">{{ $activities }}</textarea>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <small class="text-muted">Format: JPG, PNG, JPEG (Max 2MB). Biarkan kosong jika tidak ingin mengubah.</small>
                                @error('image')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
...
<script>
    ...
    // Koordinat Awal (Database / Old Value)
    const initialLat = {{ old('latitude', $destination->latitude ?? -4.68) }};
    const initialLng = {{ old('longitude', $destination->longitude ?? 104.55) }};
    ...
    function initMap() {
        ...
        document.getElementById('resetMap').onclick = () => {
            ...
            latitudeField.value = initialLat;
            longitudeField.value = initialLng;
        };
                ...
                .then(data => {
                    ...
                    data.features.forEach(place => {
                        ...
                        div.onclick = () => {
                            ...
                            // Auto fill address only if empty (prevent overwriting user edit)
